...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default='/scratch/jeh16/imagenet_ffcv', type=str,
                        help='dataset path')
    parser.add_argument('--percentage', default=1., type=float, metavar='PCT',
...
def main(args):
    ...
    data_loader_train = Loader(
        f"{args.data_path}/imagenet_train_{args.percentage:.2f}.ffcv",
    ...
    cropper = CenterCropRGBImageDecoder(res_tuple, ratio=0.875)
    image_pipeline = [
        cropper,
        ToTensor(),
        ToDevice(torch.device(args.device), non_blocking=True),
    ...
    data_loader_val = Loader(
        f"{args.data_path}/imagenet_validation.ffcv",
        batch_size=args.batch_size,
        num_workers=args.num_workers,
    ...
    model = models_vif.__dict__[args.model](
    ...
    if args.finetune and not args.eval:
        ...
        if args.global_pool:
            ...
            # assert set(msg.missing_keys) == {'head.weight', 'head.bias', 'cls_token'}
    ...
    if args.eval:
        test_stats = evaluate(data_loader_val, model, args.device, epoch=epoch)
        print(f"Accuracy of the network on the {len(data_loader_val) * eff_batch_size} test images: {test_stats['acc1']:.1f}%")
        exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if logger is not None:
            logger.log(
                {
                    'eval_top1': test_stats['acc1'],
                    'eval_top5': test_stats['acc5'],
